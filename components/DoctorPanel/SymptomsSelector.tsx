...
//   const [focusedIndex, setFocusedIndex] = useState(-1); // -1 for no focus, -2 for custom add focus
...
//         !initialSymptoms.some(is => is.toLowerCase() === symptom.toLowerCase()) // Case-insensitive check for already selected
//     ).slice(0, 7); // Slice to 7 to make space for custom add potentially
...
//         } else if (canAddCustom && focusedIndex === -1 && trimmedSearchTerm) { // If no arrow keys used, Enter on custom
//           handleSymptomSelect(trimmedSearchTerm, true);
//         }
//         break;
//       case 'Escape':
...
//       <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-4 py-4 sm:px-6 sm:py-5 md:px-8 md:py-6">
...
//               <p className="text-blue-100 text-xs sm:text-sm mt-0.5 sm:mt-1">Select all symptoms that apply</p>
//             </div>
//           </div>
//           <div className="bg-white/10 backdrop-blur-sm px-3 py-1.5 sm:px-4 sm:py-2 rounded-full border border-white/20 self-start sm:self-center mt-1 sm:mt-0">
...
//           <label htmlFor="symptom-search" className="block text-xs sm:text-sm font-semibold text-slate-700 mb-2 sm:mb-3">
...
//               className="w-full pl-10 sm:pl-12 pr-4 py-3 sm:py-3.5 bg-slate-50 border-2 border-slate-200 rounded-lg sm:rounded-xl focus:ring-2 sm:focus:ring-4 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all duration-200 text-slate-900 placeholder-slate-500 text-sm"
//               autoComplete="off"
//             />
//           </div>

//           {isDropdownOpen && trimmedSearchTerm.length > 0 && (
//             <div className="absolute z-20 w-full mt-1.5 sm:mt-2 bg-white border border-slate-200 rounded-lg sm:rounded-xl shadow-2xl overflow-hidden">
//               <ul role="listbox" className="max-h-64 overflow-y-auto"> {/* Increased max-h for custom add */}
//                 {filteredSymptoms.map((symptom, index) => (
//                   <li key={symptom} role="option" aria-selected={index === focusedIndex}>
//                     <button
//                       onClick={() => handleSymptomSelect(symptom)}
//                       className={`w-full text-left px-3 py-2.5 sm:px-4 sm:py-3 transition-all duration-150 border-b border-slate-100 last:border-b-0 text-sm ${index === focusedIndex ? 'bg-blue-50 text-blue-900' : 'hover:bg-slate-50 text-slate-700'
//                         }`}
//                       onMouseEnter={() => setFocusedIndex(index)}
//                     >
//                       <div className="flex items-center justify-between">
//                         <span className="font-medium truncate">{symptom}</span>
//                         <Plus className={`h-4 w-4 transition-colors flex-shrink-0 ${index === focusedIndex ? 'text-blue-600' : 'text-slate-400'}`} />
//                       </div>
//                     </button>
//                   </li>
//                 ))}
...
//                       className={`w-full text-left px-3 py-2.5 sm:px-4 sm:py-3 transition-all duration-150 text-sm ${filteredSymptoms.length > 0 ? 'border-t border-slate-200' : ''} ${focusedIndex === filteredSymptoms.length ? 'bg-green-50 text-green-900' : 'hover:bg-green-50 text-slate-700'
//                         }`}
//                       onMouseEnter={() => setFocusedIndex(filteredSymptoms.length)}
//                     >
//                       <div className="flex items-center justify-between">
//                         <span className="font-medium">Add: "<em>{trimmedSearchTerm}</em>"</span>
//                         <Plus className={`h-4 w-4 transition-colors flex-shrink-0 ${focusedIndex === filteredSymptoms.length ? 'text-green-600' : 'text-slate-400'}`} />
//                       </div>
//                     </button>
//                   </li>
//                 )}
//                 {filteredSymptoms.length === 0 && !canAddCustom && trimmedSearchTerm.length > 0 && (
//                   <li className="p-3 py-2.5 sm:px-4 sm:py-3 text-center text-slate-500 text-sm">
...
//         {/* Selected Symptoms Display (no changes) */}
//         <div className="space-y-3 sm:space-y-4">
//             <div className="flex items-center justify-between">
//               <label className="text-xs sm:text-sm font-semibold text-slate-700">Selected Symptoms</label>
//               {initialSymptoms.length > 0 && (
//                 <button onClick={() => onSymptomsChange([])} className="text-xs sm:text-sm text-slate-500 hover:text-red-600 transition-colors duration-200 font-medium">Clear All</button>
//               )}
//             </div>
            
//             <div className="min-h-[100px] sm:min-h-[120px] p-4 sm:p-6 bg-gradient-to-br from-slate-50 to-slate-100 rounded-lg sm:rounded-xl border-2 border-dashed border-slate-300">
//               {initialSymptoms.length > 0 ? (
//                 <div className="flex flex-wrap gap-2 sm:gap-3">
//                   {initialSymptoms.map((symptom) => (
//                     <div key={symptom} className="group inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 bg-white rounded-full border-2 border-blue-200 shadow-sm hover:shadow-md transition-all duration-200 hover:border-blue-300">
//                       <Activity className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-blue-600 mr-1.5 sm:mr-2 flex-shrink-0" />
//                       <span className="text-slate-800 font-medium text-xs sm:text-sm truncate">{symptom}</span>
//                       <button onClick={() => handleSymptomRemove(symptom)} className="ml-1.5 sm:ml-2 p-0.5 sm:p-1 hover:bg-red-100 rounded-full transition-colors duration-200 group-hover:bg-red-50" aria-label={`Remove ${symptom}`}>
//                         <X className="h-3 w-3 sm:h-3.5 text-slate-500 hover:text-red-600" />
//                       </button>
//                     </div>
//                   ))}
//                 </div>
...
//                   <div className="bg-slate-200 p-3 sm:p-4 rounded-full mb-3 sm:mb-4">
//                     <Search className="h-6 w-6 sm:h-8 sm:w-8 text-slate-400" />
//                   </div>
//                   <p className="text-slate-500 font-medium text-sm sm:text-base">No symptoms selected</p>
//                   <p className="text-slate-400 text-xs sm:text-sm mt-1">Use search to find and add symptoms</p>
//                 </div>
//               )}
//             </div>
//           </div>
...
const SymptomsSelector = ({ initialSymptoms, onSymptomsChange }: SymptomsSelectorProps) => {
  ...
  return (
    <div className="bg-slate-800/40 rounded-xl sm:rounded-2xl shadow-xl border border-slate-700/70 overflow-hidden">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 px-4 py-4 sm:px-6 sm:py-5 md:px-8 md:py-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div>
              <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-white">Symptom Assessment</h2>
              <p className="text-blue-200 text-xs sm:text-sm mt-0.5 sm:mt-1">Select all symptoms that apply</p>
            </div>
          </div>
        ...
        <div className="relative mb-6 sm:mb-8" ref={dropdownContainerRef}>
          ...
          <div className="relative">
            ...
            <input
              ...
              onKeyDown={handleKeyDown}
              placeholder="Type to search symptoms..."
              className="w-full pl-10 sm:pl-12 pr-4 py-3 sm:py-3.5 bg-slate-700/50 border-2 border-slate-600 rounded-lg sm:rounded-xl focus:ring-2 sm:focus:ring-4 focus:ring-blue-500/30 focus:border-blue-500 outline-none transition-all duration-200 text-slate-100 placeholder-slate-400 text-sm"
              autoComplete="off"
            />
          </div>
              ...
              <ul role="listbox" className="max-h-64 overflow-y-auto">
                {filteredSymptoms.map((symptom, index) => (
                  <li key={symptom} role="option" aria-selected={index === focusedIndex}>
                    <button
                      onClick={() => handleSymptomSelect(symptom)}
                      className={`w-full text-left px-3 py-2.5 sm:px-4 sm:py-3 transition-all duration-150 border-b border-slate-700 last:border-b-0 text-sm ${index === focusedIndex ? 'bg-blue-600/70 text-white' : 'hover:bg-slate-700/60 text-slate-200'
                        }`}
                      onMouseEnter={() => setFocusedIndex(index)}
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-medium truncate">{symptom}</span>
                        <Plus className={`h-4 w-4 transition-colors flex-shrink-0 ${index === focusedIndex ? 'text-blue-200' : 'text-slate-400'}`} />
                      </div>
                    </button>
                  </li>
                ))}
        ...
        <div className="space-y-3 sm:space-y-4">
            ...
            <div className="min-h-[100px] sm:min-h-[120px] p-4 sm:p-6 bg-slate-700/30 rounded-lg sm:rounded-xl border-2 border-dashed border-slate-600">
              {initialSymptoms.length > 0 ? (
                <div className="flex flex-wrap gap-2 sm:gap-3">
                  {initialSymptoms.map((symptom) => (
                    <div key={symptom} className="group inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 bg-slate-700/60 rounded-full border-2 border-blue-700/50 shadow-sm hover:shadow-md transition-all duration-200 hover:border-blue-500/70">
                      <Activity className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-blue-400 mr-1.5 sm:mr-2 flex-shrink-0" />
