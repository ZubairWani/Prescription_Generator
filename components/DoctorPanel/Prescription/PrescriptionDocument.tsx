...
// --- FONT STRATEGY ---
// Use one base font family and apply modifiers (fontWeight, fontStyle)
const FONT_FAMILY_BASE = "Helvetica"; // Standard PDF font

// If you were to use custom fonts, you'd register them here:
// Font.register({
//   family: 'OpenSans',
//   fonts: [
...
//   ]
// });
// And then FONT_FAMILY_BASE would be 'OpenSans'
...
// Custom SVG Icons (Your SVGs are kept as is)
const MedicalIcon = () => (
...
// Template Configurations (kept as is)
const getTemplateConfig = (template: PrescriptionTemplate) => {
  const configs = {
    ...
    modern: { primaryColor: "#10b981", secondaryColor: "#d1fae5", accentColor: "#059669", icon: <StethoscopeIconPdf />, headerBg: "#10b981", footerBg: "#10b981", borderColor: "#10b981", hasTopBorder: true, hasBottomBorder: true, gradientFooter: false },
...
// Dynamic Styles Function - Updated with FONT_FAMILY_BASE strategy
const createDynamicStyles = (template: PrescriptionTemplate) => {
  const config = getTemplateConfig(template);
  return StyleSheet.create({
    page: {
      fontFamily: FONT_FAMILY_BASE, // Applied base font
      fontSize: 10,
      paddingTop: 30,
      paddingLeft: 40,
      paddingRight: 40,
    ...
    hospitalName: { fontSize: 20, fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", color: config.primaryColor, marginBottom: 4, letterSpacing: 0.5 },
    hospitalSlogan: { fontSize: 10, color: "#6b7280", fontStyle: "italic", marginBottom: 8, fontFamily: FONT_FAMILY_BASE },
    doctorName: { fontSize: 14, fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", color: "#111827" },
    doctorQualification: { fontSize: 10, color: "#6b7280", fontFamily: FONT_FAMILY_BASE },
    ...
    patientInfoText: { fontSize: 9, color: "#374151", borderBottomWidth: 1, borderBottomColor: "#d1d5db", paddingBottom: 2, minWidth: 100, fontFamily: FONT_FAMILY_BASE },
    rxSymbol: { fontSize: 28, fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", color: config.accentColor, marginTop: 15, marginBottom: 5 },
    ...
    footerText: { fontSize: 8, color: config.hasBottomBorder ? "#ffffff" : "#6b7280", textAlign: "center", lineHeight: 1.3, fontFamily: FONT_FAMILY_BASE },
    addressText: { fontSize: 8, color: config.hasBottomBorder ? "#ffffff" : "#6b7280", textAlign: "center", marginBottom: 4, fontFamily: FONT_FAMILY_BASE },
    contactText: { fontSize: 8, color: config.hasBottomBorder ? "#ffffff" : "#6b7280", textAlign: "center", fontFamily: FONT_FAMILY_BASE },
    ...
    sectionTitle: { fontSize: 11, fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", color: "#374151", marginLeft: 6, textTransform: "uppercase", letterSpacing: 0.3 },
    ...
    fieldLabel: { fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", color: "#6b7280", fontSize: 8, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 2 },
    fieldValue: { color: "#111827", fontSize: 10, fontFamily: FONT_FAMILY_BASE, fontWeight: "bold" }, // Default to bold for values, adjust if needed
    fieldValueRegular: { color: "#111827", fontSize: 10, fontFamily: FONT_FAMILY_BASE }, // For non-bold values
    badgeContainer: { flexDirection: "row", flexWrap: "wrap", marginHorizontal: -3 },
    badge: { backgroundColor: "#fef3c7", color: "#92400e", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 15, fontSize: 9, fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", borderWidth: 1, borderColor: "#f59e0b", marginHorizontal: 3, marginVertical: 3 },
    ...
    tableHeaderText: { color: "#ffffff", fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", fontSize: 9, textAlign: "center", textTransform: "uppercase", letterSpacing: 0.5 },
    ...
    tableCell: { fontSize: 9, color: "#374151", paddingHorizontal: 4, textAlign: "center", fontFamily: FONT_FAMILY_BASE },
    tableCellLeft: { textAlign: "left" },
    tableCellBold: { fontFamily: FONT_FAMILY_BASE, fontWeight: "bold" },
    ...
    diagnosisText: { fontSize: 12, color: "#111827", fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", lineHeight: 1.5 },
    contentText: { fontSize: 10, color: "#374151", lineHeight: 1.6, backgroundColor: "#f8fafc", padding: 12, borderRadius: 6, borderLeftWidth: 3, borderLeftColor: config.primaryColor, fontFamily: FONT_FAMILY_BASE },
    notesText: { fontSize: 10, color: "#374151", lineHeight: 1.6, marginTop: 4, fontFamily: FONT_FAMILY_BASE },
    noData: { fontStyle: "italic", color: "#9ca3af", fontSize: 9, fontFamily: FONT_FAMILY_BASE },
    familyDiseaseItem: { marginBottom: 6 },
    familyDiseaseName: { fontFamily: FONT_FAMILY_BASE, fontWeight: "bold", fontSize: 9, color: "#374151" },
    familyDiseaseRelation: { fontStyle: "italic", fontSize: 8, color: "#6b7280", marginLeft: 5, fontFamily: FONT_FAMILY_BASE },
    familyDiseaseDetails: { fontSize: 9, color: "#4b5563", marginLeft: 10, marginTop: 2, fontFamily: FONT_FAMILY_BASE },
  });
};
...
const PrescriptionDocument: React.FC<PrescriptionDocumentProps> = ({
  ...
  const generatePrescriptionId = () => {
    return (
      prescriptionId ||
      `RX-${new Date().getFullYear()}-${Math.random().toString(36).substr(2, 8).toUpperCase()}`
    );
  };
  ...
  const renderTextOrNA = ( text: string | number | undefined | null, defaultText = "Not specified", style?: object ) => {
    if (text === undefined || text === null || String(text).trim() === "") {
      return <Text style={{ ...styles.noData, ...style }}>{defaultText}</Text>;
    }
    // If a specific style for fieldValue (like bold) is desired, apply it. Otherwise, use a regular style.
    // For simplicity, let's assume default values are bold as per your original `fieldValue`
    return <Text style={{ ...styles.fieldValue, ...style }}>{String(text)}</Text>;
  };
  
  // This function is mainly for direct string concatenation, ensure its output is always inside a <Text>
  const renderStringOrNA = ( text: string | number | undefined | null, defaultText = "........................." ) => {
    if (text === undefined || text === null || String(text).trim() === "") {
      return defaultText;
    }
    return String(text);
  ...
  // Helper to ensure content is wrapped in <Text> if it's a non-empty string
  const TextWrapper = ({ children }: { children: React.ReactNode }) => {
    if (typeof children === 'string' && children.trim() !== '') {
      return <Text>{children}</Text>;
    }
    if (React.isValidElement(children) && children.type === Text) {
      return children; // Already a Text component
    }
    // If it's some other valid element or empty, render as is (might need more checks for complex cases)
    // For safety, if it's not a Text component and not explicitly handled, wrap it if it's not null/undefined
    if (children !== null && children !== undefined && !React.isValidElement(children)) {
        // This case is tricky, ideally all text content is explicitly string or <Text>
        // For now, if it's a primitive that's not a string, stringify it inside Text
        return <Text>{String(children)}</Text>;
    }
    return <>{children}</>; // Render valid elements or null/undefined
  };
    ...
    <Page size="A4" style={styles.page}>
      <View style={styles.headerContainer}>
        ...
        <View style={styles.patientInfoLine}>
          ...
          <Text style={styles.patientInfoText}>
            Age: {data.age ? `${data.age}` : renderStringOrNA(null, "......")}
          </Text>
          <Text style={styles.patientInfoText}>
            Sex: {data.gender || renderStringOrNA(null, "......")}
          </Text>
          <Text style={styles.patientInfoText}>
      ...
      <View style={styles.contentContainer}>
        <View style={styles.contentArea}>
            {/* Add default content or leave empty. Ensure no loose strings. */}
            {/* Example: <Text style={styles.noData}>No specific content for simple view.</Text> */}
        </View>
      </View>
  ...
  const renderDetailedTemplate = () => (
    <Page size="A4" style={styles.page}>
      <View style={styles.headerContainer} fixed> {/* `fixed` can help keep header on each page if content overflows */}
        <View style={styles.headerTop}>
          <View style={styles.leftHeaderSection}>
            <Text style={styles.hospitalName}>{hospitalName}</Text>
            <Text style={styles.hospitalSlogan}>{hospitalSlogan}</Text>
      ...
      <View style={styles.contentContainer}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.sectionContent}>
              <View style={styles.patientGrid}>
                ...
                {(() => {
                  const weightStr = data.weight != null ? String(data.weight) : undefined;
                  const heightStr = data.height != null ? String(data.height) : undefined;
                  const bmiResult = calculateBMI(weightStr, heightStr);
                  return bmiResult && (<View style={styles.patientField}><View style={styles.fieldContainer}><Text style={styles.fieldLabel}>BMI</Text>{renderTextOrNA(bmiResult)}</View></View>);
                })()}
              </View>
            </View>
          </View>
        ...
        {/* Family History */}
        {(data.familyHistory?.commonDiseases?.some(d => d?.diseaseName?.trim()) || (data.familyHistory?.familyHistoryNotes && data.familyHistory.familyHistoryNotes.trim() !== "")) && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}><HistoryIcon /><Text style={styles.sectionTitle}>Family Medical History</Text></View>
            <View style={styles.sectionContent}>
              {data.familyHistory.commonDiseases && data.familyHistory.commonDiseases.filter(d => d?.diseaseName?.trim()).length > 0 && (
                <>
                  <Text style={styles.fieldLabel}>Reported Conditions:</Text>
                  {data.familyHistory.commonDiseases.filter(d => d?.diseaseName?.trim()).map((disease, index) => (
                    <View key={`family-disease-${index}`} style={styles.familyDiseaseItem}>
                      <Text style={styles.familyDiseaseName}>
                        <TextWrapper>{disease.diseaseName}</TextWrapper>
                        {disease.inheritance && <Text style={styles.familyDiseaseRelation}> (<TextWrapper>{disease.inheritance}</TextWrapper>)</Text>}
                      </Text>
                      {disease.details && disease.details.trim() && <Text style={styles.familyDiseaseDetails}>- <TextWrapper>{disease.details.trim()}</TextWrapper></Text>}
                    </View>
                  ))}
                </>
              )}
              {data.familyHistory.familyHistoryNotes && data.familyHistory.familyHistoryNotes.trim() && (
                <>
                  <Text style={[styles.fieldLabel, { marginTop: data.familyHistory.commonDiseases?.filter(d => d?.diseaseName?.trim()).length > 0 ? 8 : 0 }]}>Additional Notes:</Text>
      ...
      <View style={styles.footerContainer} fixed> {/* `fixed` can help keep footer on each page */}
  ...
  return (
    <Document author={doctorName || "MedScript Pro"} title={`Prescription for ${data.fullName || 'Patient'}`}>
