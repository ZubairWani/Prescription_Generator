...
//   initialLabTests: string[]; // Lab tests passed from parent
//   onLabTestsChange: (updatedTests: string[]) => void; // Callback to update parent
// }
...
//   const searchRef = useRef<HTMLInputElement>(null); // Typed ref

//   const filteredTests = predefinedTests.filter(test =>
//     test.toLowerCase().includes(searchQuery.toLowerCase()) &&
//     !initialLabTests.includes(test) // Check against parent's list
//   );
...
//       onLabTestsChange(newSelectedTests); // Update parent state
//     }
//     setSearchQuery('');
//     setIsDropdownOpen(false);
//   };
...
//       // Optionally, notify user that the custom test is already selected
//       // console.log("Custom test already selected.");
//       setCustomTest(''); // Clear input even if already selected
//     }
//   };
...
//         // If Enter is pressed on a search query that's not in predefined and not already selected, add it as custom.
...
//         <label className="text-xl font-semibold flex items-center text-gray-800">
...
//                   setIsDropdownOpen(e.target.value.length > 0 || filteredTests.length > 0); // Open if typing or results exist
//                 }}
//                 onFocus={() => setIsDropdownOpen(searchQuery.length > 0 || filteredTests.length > 0)}
//                 onKeyDown={handleSearchKeyPress} // Changed from onKeyPress for better React event handling
//                 placeholder="Search for lab tests or imaging..."
//                 className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-md focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
//               />
//               <button 
//                 type="button" 
...
//           {/* Dropdown Menu */}
//           {isDropdownOpen && (searchQuery.length > 0 || filteredTests.length > 0) && ( // Only show if there's a query or results
//             <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
//               {filteredTests.length > 0 ? (
//                 filteredTests.map((test) => ( // Use test as key since it's unique in this filtered list
//                   <div
//                     key={test}
...
//       {/* Custom Test Input */}
//       <div className="mb-4">
//         <label htmlFor="customLabTest" className="block text-sm font-medium text-gray-700 mb-1">Add Custom Test:</label>
//         <div className="flex gap-2">
//           <input
//             id="customLabTest"
//             type="text"
...
//             onKeyDown={handleCustomKeyPress} // Changed from onKeyPress
//             placeholder="Type custom test and press Enter or click Add"
//             className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
//           />
//           <button
...
//       {/* Selected Tests Tags */}
//       {initialLabTests.length > 0 && ( // Use prop for count and iteration
//         <div className="mb-4">
//           <h4 className="text-sm font-medium text-gray-700 mb-2">Selected Tests ({initialLabTests.length}):</h4>
//           <div className="flex flex-wrap gap-2">
//             {initialLabTests.map((test) => ( // Iterate over prop
//               <span
//                 key={test} // Use test as key
...
//       {/* Clear All Button */}
//       {initialLabTests.length > 0 && ( // Use prop for condition
//         <div className="flex justify-end">
//           <button
//             type="button"
//             onClick={() => onLabTestsChange([])} // Clear all by calling parent
//             className="px-4 py-2 text-sm bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 border border-gray-300"
//           >
//             Clear All Tests
//           </button>
//         </div>
    ...
    <section className="bg-slate-800/40 p-4 sm:p-6 rounded-xl border border-slate-700/70 shadow-md">
      <div className="flex items-center justify-between mb-4 sm:mb-5">
        <label className="text-lg sm:text-xl font-semibold flex items-center text-slate-100">
          <FlaskConical className="h-5 w-5 mr-2.5 text-blue-400" />
          MEDICAL LAB TESTS / IMAGING SUGGESTED
        </label>
      </div>
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                ...
                placeholder="Search for lab tests or imaging..."
                className="w-full pl-10 pr-10 py-2.5 sm:py-3 bg-slate-700/50 border border-slate-600 text-slate-100 placeholder-slate-400 rounded-lg sm:rounded-xl focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 outline-none text-sm"
              />
              <button
                type="button"
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
              ...
              {filteredTests.length > 0 ? (
                filteredTests.map((test) => (
                  <div
                    key={test}
                    onClick={() => handleTestSelect(test)}
                    className="px-3 py-2 hover:bg-blue-600/30 cursor-pointer text-sm text-slate-200 border-b border-slate-700 last:border-b-0"
                  >
                    {test}
                  </div>
                ))
      ...
      <div className="mb-4 sm:mb-5">
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Type custom test and press Enter or click Add"
            className="flex-1 px-3 py-2.5 sm:py-3 bg-slate-700/50 border border-slate-600 text-slate-100 placeholder-slate-400 rounded-lg sm:rounded-xl focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 outline-none text-sm"
          />
          <button
            type="button"
            onClick={handleCustomTestAdd}
            disabled={!customTest.trim()}
            className="px-4 py-2.5 sm:py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-lg sm:rounded-xl disabled:from-slate-500 disabled:to-slate-600 disabled:cursor-not-allowed flex items-center gap-1 text-sm font-medium shadow-md hover:shadow-lg transition-all"
          >
            <Plus className="h-4 w-4" />
            Add
          </button>
        ...
        <div className="mb-4 sm:mb-5">
          ...
          <div className="flex flex-wrap gap-2">
            {initialLabTests.map((test) => (
              <span
                key={test}
                className="inline-flex items-center px-3 py-1.5 rounded-full text-sm bg-blue-700/40 text-blue-200 border border-blue-600/50 shadow-sm"
              >
                {test}
                <button
                  type="button"
